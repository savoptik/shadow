             ...
             gid_t old_gid, gid_t new_gid)
{
	...
	if (mknodat(dst->dirfd, dst->name, statp->st_mode & ~07700U, statp->st_rdev) == -1)
		return -1;

	if (chownat_if_needed(dst, statp, old_uid, new_uid, old_gid, new_gid) == -1)
		return -1;

	if (fchmodat(dst->dirfd, dst->name, statp->st_mode & 07700, AT_SYMLINK_NOFOLLOW) == -1)
		return -1;
...
#endif				/* WITH_SELINUX */
	ofd = openat (dst->dirfd, dst->name, O_WRONLY | O_CREAT | O_EXCL | O_TRUNC | O_NOFOLLOW | O_CLOEXEC, statp->st_mode & 07700);
	if (   (ofd < 0)
